...
%See also: labMetaData, orientedLabTimeSeries, labTimeSeries   
    ...
    methods
        ...
        %Constructor:
        function this=labData(metaData,markerData,EMGData,GRFData,beltSpeedSetData,beltSpeedReadData,accData,EEGData,footSwitches, ppData)
            %----------------
            ...
            if nargin<8 || isempty(ppData)
                this.ppData=[];
            elseif isa(ppData,'labTimeSeries')
                this.ppData=ppData; %Needs to be empty or have labels in the international 10-20 system.
            else
                ME=MException('labData:Constructor','Tenth argument (ppData) should be a LabTimeSeries object.');
                throw(ME);
            end
            %---------------
        ...
        function partialppData=getppData(this,label)
            partialppData=this.getPartialData('ppData',label);
        end
        
        function list=getppList(this)
            list=this.getLabelList('ppData');
        end
