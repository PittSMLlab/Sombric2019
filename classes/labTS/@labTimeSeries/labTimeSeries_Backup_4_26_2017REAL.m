    ...
    methods(Static)
        ...
        function [alignedTS,originalDurations]=stridedTSToAlignedTS(stridedTS,N) %Need to correct this, so it aligns by all events, as opposed to just aligning the initial time-point
            ...
            if ~islogical(stridedTS{1}.Data)
                if ~isempty(stridedTS{1}.Data)
                aux=zeros(sum(N),size(stridedTS{1}.Data,2),size(stridedTS,1));
                rat=1;
                elseif ~isempty(stridedTS{2}.Data)
                aux=zeros(sum(N),size(stridedTS{2}.Data,2),size(stridedTS,1));
                rat=2;
                else
                    aux=zeros(sum(N),size(stridedTS{3}.Data,2),size(stridedTS,1));
                    rat=3;
                end
            else
            ...
            for i=1:Nstrides %Going over strides
                ...
                for j=1:Nphases %Going over aligned phases
                   % if isa(stridedTS{i,j},'labTimeSeries') %Origonal
                    if isa(stridedTS{i,j},'labTimeSeries')% &&  ~isempty(stridedTS{i,j}.timeRange) %CJS update
...
%                                 aux(M(j)+1:M(j+1),:,i)=aa.Data;
                            if length(find(isnan(stridedTS{i,j}.Data)==0))<=3 %CJS 4/19/2017 New
                                %aux(M(j)+1:M(j+1),:,i)=NaN.*ones(length(M(j)+1:M(j+1)), 3);
                                aux(M(j)+1:M(j+1),:,i)=NaN.*ones(length(M(j)+1:M(j+1)), size(aux, 2));
                            else
                                aa=resampleN(stridedTS{i,j},N(j));
                                aux(M(j)+1:M(j+1),:,i)=aa.Data;
                            end
            ...
            alignedTS=alignedTimeSeries(0,1/sum(N),aux,stridedTS{rat}.labels,N,cell(size(N)));
            else
