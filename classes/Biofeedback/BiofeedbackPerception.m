                ...
                if iscell(filename)%if more than one file is selected for analysis
                    ...
                    for z = 1:length(filename)
                        ...
                        for t=1:length(rts{z})
                            ...
                            if rts{z}(t)==PossibleTarget(1)
                                RDATA(t)=1;
                            elseif rts{z}(t)==PossibleTarget(2)
                                RDATA(t)=2;
                            elseif rts{z}(t)==PossibleTarget(3)
                                RDATA(t)=3;
                            else
                                break
                            end
                        ...
                        for t=1:length(lts{z})
                            ...
                            if lts{z}(t)==PossibleTarget(1)
                                LDATA(t)=1;
                            elseif lts{z}(t)==PossibleTarget(2)
                                LDATA(t)=2;
                            elseif lts{z}(t)==PossibleTarget(3)
                                LDATA(t)=3;
                            else
                                break
                            end
                    ...
                    %%%DDR{1}=RR{wash}-repmat([mean(RR{adapt})], size(RR{wash}, 1), 1);%DDR{1}(1:end-1, 2)=0;% Use the whole Baseline, mean
                    ...
                    %%%DDL{1}=LL{wash}-repmat([mean(LL{adapt})], size(LL{wash}, 1), 1);%DDL{1}(1:end-1, 2)=0;% Use the whole Baseline, mean
        ...
        function [rhits, lhits, rts, lts, color,rv,lv]=getHits(this)
            ...
            if iscell(filename)%if more than one file is selected for analysis
                ...
                for z = 1:length(filename)
                    ...
                    Lgamma2 = data2(:,11);
                    target = data2(:,14);
                    if strcmp(this.triallist{z,4},'Familiarization') &&  size(data2, 2)>14
                        visible = data2(:,15);
                    end
        ...
        function []=PlotErrorTimecourse(this, rhits, lhits, rts, lts, rv,lv)
            ...
            for z = 1:length(filename)
                ...
                if strcmp(this.triallist{z,4},'Familiarization')
                    ...
                    temp2 = lts{z};
                    groupies = temp+temp2;
                    g=gscatter([1:length(lhits{z})]+h,lhits{z},groupies,['b', 'k', 'r','b', 'k', 'r'],['s','o','d','s','o','d']);
                    colorfull=['w','w', 'w', 'b', 'k' ,'r' ];
                    colorout=['b','k', 'r', 'k', 'k' ,'k' ];
                    for y = 1:length(g)
                        set(g(y),'MarkerFaceColor',colorfull(y));  
