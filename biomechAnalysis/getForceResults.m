...
for g=1:ngroups
    ...
    for s=1:length(subjects)
        ...
        %FirstSteps=[FirstSteps; nanmean(SBaseData(1:5,:))];
        ...
        %ShortExposure=[ShortExposure; nanvar(ShortExp(:,:))];
        ...
        %         EarlyA=[EarlyA; nanmean(AData(6:10,:))];%NORMAL WAY, conservative
        ...
        %EarlySlope=[EarlySlope; nanmean(AData(1:5,:))];
        %EarlySlope=[EarlySlope; nanmean(AData(1,:))];
        %EarlySlope=[EarlySlope; (nanmean(AData(11:15,:))-nanmean(AData(1:5,:)))/4];
        %EarlySlope=[EarlySlope; (nanmean(AData(26:30,:))-nanmean(AData(1:5,:)))/4];
        %EarlySlope=[EarlySlope; (nanmean(AData(21:25,:))-nanmean(AData(6:10,:)))/4];
        ...
        %EarlyAWBias=[ EarlyAWBias; nanmean(ADataWBias(6:10, :))];
        %EarlyAWBias=[ EarlyAWBias; nanmean(ADataWBias(1:5, :))];
        EarlyShortExpoWBias=[ EarlyShortExpoWBias; nanmean(ShortExp(1:5, :))];
        %          EarlyShortExpoWBias=[ EarlyShortExpoWBias; nanmean(ShortExp(6:end, :))];
        %EarlyShortExpoWBias=[ EarlyShortExpoWBias; nanmean(ShortExp((end)-5+1:(end), :))];
        %EarlyShortExpoWBias=[ EarlyShortExpoWBias; nanmedian(ShortExp(:, :))];
        ...
        if strcmp(groups(g), 'InclineStroke')
            ...
            %CJS: Old way to do this that doesn't give the sign I want
            %             DelFDeAdapt2Base=[DelFDeAdapt2Base; baseWBias-nanmean(PDataWBias(1:5,:))];
        ...
        %BaseAdapDiscont=[BaseAdapDiscont; nanmean(AData(6:10,:))-nanmean(MBaseData(6:end-4, :))];
        BaseAdapDiscont=[BaseAdapDiscont; nanmean(AData(6:10,:))-Realbase];
        %BaseAdapDiscont=[BaseAdapDiscont; nanmean(MBaseData(6:end-4, :))];
        %BaseAdapDiscont=[BaseAdapDiscont; nanmean(AData(1:5,:))];
        %BaseAdapDiscont=[BaseAdapDiscont; nanmean(AData(6:10,:))];
        SS_PA_Discont=[SS_PA_Discont; nanmean(AData((end-5)-steadyNumPts+1:(end-5),:))-nanmean(PData(1:5, :))];
        
        %getEarlyLateData_v2(this.removeBadStrides,labels,baseCond,0,-40,5,10); %Last 40, exempting very last 5 and first 10
        
        BaseShortExposureDiscont=[BaseShortExposureDiscont; nanmean(ShortExp(1:5,:))-nanmean(MBaseData(6:end-4, :))];
        %        BaseShortExposureDiscont=[BaseShortExposureDiscont; nanmean(ShortExp(:,:))-nanmean(MBaseData(6:end-4, :))];
        %         BaseShortExposureDiscont=[BaseShortExposureDiscont; nanmedian(ShortExp(:,:))-nanmean(MBaseData(6:end-4, :))];
        %         BaseShortExposureDiscont=[BaseShortExposureDiscont; nanmean(ShortExp((end)-5+1:(end),:))-nanmean(MBaseData(6:end-4, :))];
        ...
        %SpeedAdapDiscont=[SpeedAdapDiscont; nanmean(ADataWBias(end-44:end-5, :))-speedBias];EarlyStrides
    ...
    if ~isempty(Breaking)
        ...
        %DelFAPA(:, Breaking)=-1.*DelFAPA(:, Breaking);
...
if nargin>4 && plotFlag
    ...
    %epochs={'SlowBase','FastBase', 'MidBase','DelFAdapt','BaseAdapDiscont'};%, 'BasePADiscont'};
    ...
    %epochs={'EarlyA', 'DelFAdapt','TMafter', 'DelFDeAdapt', 'DelFDeAdapt2Base', 'DelFDeAdapt2Base'};
    %epochs={'SlowMediumBaseDiscont', 'FastMediumBaseDiscont', 'RatioBaseDiscont'};
    %epochs={'SlowBasePosVar', 'FastBasePosVar', 'TMSteadyPosVar', 'AllAdaptPosVar'};
    %epochs={'BaseAdapDiscont', 'SpeedAdapDiscont', 'SpeedSSDiscont', 'SpeedPADiscont'};
    %epochs={'TMSteadyWBias', 'TMafterWBias', 'SS_PA_Discont','BaseAdapDiscont', 'DelFAdapt','DelFDeAdapt'};
    %epochs={'TMSteady', 'EarlyA','BaseAdapDiscont', 'SS_PA_Discont', 'DelFAdapt','DelFDeAdapt'};
   %epochs={'SlowBase','FastBase', 'SpeedAdapDiscont', 'SpeedSSDiscont', 'SS_PA_Discont'};
