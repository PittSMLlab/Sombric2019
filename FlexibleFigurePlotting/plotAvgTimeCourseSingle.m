function PlottedMean=plotAvgTimeCourseSingle(adaptDataList,params,conditions,binwidth,trialMarkerFlag,indivFlag,indivSubs,colorOrder,biofeedback,removeBiasFlag,labels,medianFlag,plotHandles, rowPLOT, col, sub,normStrideLflag, afterTrialP, earlyValues,lateValues)
...
if nargin<17 || isempty(normStrideLflag)
    normStrideLflag=[];
end

if nargin<18 || isempty(afterTrialP)
    afterTrialP=5;
end

if nargin<19 || isempty(earlyValues)     
     earlyValues=[];
...
if nargin<20 || isempty(lateValues)   
    lateValues=[];
    ...
    for subject=1:length(adaptDataList{group})
        ...
        for c=1:nConds
            ...
            for t=1:length(trials)
                ...
                if isempty(normStrideLflag)
                    dataPts=adaptData.getParamInTrial(params,trials{t});
                elseif ~isempty(normStrideLflag)
                    tr=adaptData.getTrialsInCond({'TM base'});
                    normBaseSL=nanmean(adaptData.getParamInTrial({'stepLengthSlow'},tr)+adaptData.getParamInTrial({'stepLengthFast'},tr));                                       
                    dataPts=adaptData.getParamInTrial(params,trials{t})./normBaseSL;
                end
    ...
    for c=1:nConds
        for t=1:length(fields(values(group).(params{p}).(cond{c})));
            ...
            if maxNumPts
                ...
                if strcmp(cond{c},'TMpost') %Some stroke subjects have a very short post-adaptation 150 ignore MGR 05/09/2019
                    for i=1:length(numPts.(cond{c}).(['trial' num2str(t)])) %If the minimun is less than 150 ignore MGR 05/09/2019
                        
                        if numPts.(cond{c}).(['trial' num2str(t)])(1,i)<100
                            numPts.(cond{c}).(['trial' num2str(t)])(1,i)=NaN;
                        end
            ...
            for p=1:length(params)
                ...
                if strcmp(cond{c}, 'TMbase') %MGR 05/09/2019 to plot only the baseline bias that is removed
                    
                    allValues=[];
                    for row=1:size(values(group).(params{p}).(cond{c}).(['trial' num2str(t)]),1)
                        
                        temp=values(group).(params{p}).(cond{c}).(['trial' num2str(t)])(row,(~isnan(values(group).(params{p}).(cond{c}).(['trial' num2str(t)])(row,:))));
                        
                        if isempty(temp)% CJS 5/13/2019
                            allValues(row,:)= NaN.*ones(1, size(allValues, 2));
                        else
                        allValues(row,:)=temp(end-44:end-5);
                        end
                        
                    end
                else %What is on this else is what was previously in the code, before I added the if MGR 05/09/2019
                ...
                afterTrialPad=5; %adds empty sapce to end of %Commented by MGR 05-22-2019
                y=[avg(group).(params{p}).(cond{c}).(['trial' num2str(t)]), NaN(1,afterTrialPad)]; %Commented by MGR 05-22-2019
                E=[se(group).(params{p}).(cond{c}).(['trial' num2str(t)]), NaN(1,afterTrialPad)]; %Commented by MGR 05-22-2019
                condLength=length(y); %Commented by MGR 05-22-2019
                x=Xstart:Xstart+condLength-1; %Commented by MGR 05-22-2019

                %New CJS 5/13/2019
                if group==1 && sum(strcmp(conditions, 'adaptation'))~=0 && find(strcmp(conditions, 'adaptation'))==c % 
                    blah=gca;
                    YRanger=blah.YLim;
                    vert=[x(1) YRanger(1); x(end) YRanger(1); x(end) YRanger(2);x(1) YRanger(2); x(1) YRanger(1)];
                    patch('Faces',[1:4],'Vertices',vert,'FaceColor',[.6 .6 .6])
                end
                %
                %Biofeedback
