function CorrelationsSingleXYNoIntercept(SMatrix, Y,X, groups, things, paramLabels, colorOrder, row, col, num, FigureNum,  led,epochLabels, PairedData)
...
lm = fitlm(tbl,'linear','Intercept',false);
Y_fit=lm.Fitted;
...
% if g<=2
    ...
    label1B = sprintf('Lin: R^2 = %0.2f (p_{rho} = %0.3f) \n slope=%0.2f [%0.2f, %0.2f]', Rsquared1, PVAL_Pearson, lm.Coefficients{1,1}, CISlope(1), CISlope(2));
% else
