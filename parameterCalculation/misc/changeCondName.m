...
for s=1:length(subID)
    ...
    for c=1:length(oldNames)
        ...
        if isempty(ind)
            ...
            if length(ind)>1
                question=['Do you want to replace (1) ', adaptData.metaData.conditionName{ind(1)}, ' or (2) ', adaptData.metaData.conditionName{ind(2)}, ' with ', oldNames{c}, '?']
                xxx = input(question );
                adaptData.metaData.conditionName{ind(xxx)}=newNames{c}; 
            else
