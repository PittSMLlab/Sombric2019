function [rotatedMarkerData,sAnkFwd,fAnkFwd,sAnk2D,fAnk2D,sAngle,fAngle,direction,hipPosSHS,sAnk_fromAvgHip,fAnk_fromAvgHip, hipPos3D]=getKinematicData(eventTimes,markerData,angleData,s)
...
%Commented by MGR 05/09/2019
% hipPosSHS=hipPosFwd(:,1);
% hipPosSTO=hipPosFwd(:,4);
% hipPosFHS=hipPosFwd(:,3);
% hipPosFTO=hipPosFwd(:,6);

%CJS this is where you should make changes to see if the instaneous hip
%position matters
hipPosSHS=hipPosFwd(:,1); %MGR 05/09/2019
...
% hipPosAvg_forFast = mean(nanmean(hipPosFwd(:,1))); % Average Hip Position from SHS to STO2
% hipPosAvg_forSlow = mean(nanmean(hipPosFwd(:,3))); % Average Hip Position from SHS to STO2
