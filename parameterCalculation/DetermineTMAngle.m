...
if ~isempty(findstr(trial, 'deg')) || ~isempty(findstr(trial, '8.5')) %|| ~isempty(findstr(trial, '5'))%( ~iscell(cell2mat(regexp(trial, 'deg'))))|| ~iscell(cell2mat(regexp(trial, '8.5'))) %~isempty(cell2mat(regexp(trial, 'deg'))) || ~isempty(cell2mat(regexp(trial, '8.5'))) 
    if ~isempty(findstr(trial, '8.5 deg incline')) || ~isempty(findstr(trial, '8.5 deg uphill')) || ~isempty(findstr(trial, '8.5 deg up'))%(~iscell(regexp(trial, '8.5 deg incline')) && ~isempty(cell2mat(regexp(trial, '8.5 deg incline')))) || (~iscell(regexp(trial, '8.5 deg uphill')) && ~isempty(cell2mat(regexp(trial, '8.5 deg uphill')))) %~isempty(cell2mat(regexp(trial, '8.5 deg incline'))) || ~isempty(cell2mat(regexp(trial, '8.5 deg uphill'))) %|| (~isempty(regexp(trial, '8.5 deg'))
        ang=8.5;
    elseif ~isempty(findstr(trial, '8.5 deg decline')) || ~isempty(findstr(trial, '8.5 decline')) || ~isempty(findstr(trial, '8.5 deg downhill')) %(~iscell(regexp(trial, '8.5 deg decline')) ) || (~iscell(regexp(trial, '8.5 deg downhill')) )|| (~iscell(regexp(trial, '8.5 decline'))) % || || 
        ...
        ang=-8.5;
    elseif ~isempty(findstr(trial, '5 deg incline'))|| ~isempty(findstr(trial, '5 deg uphill')) || ~isempty(findstr(trial, '5 deg'))|| ~isempty(findstr(trial, '5 deg up'))
        ang=5;
    elseif ~isempty(findstr(trial, '8 deg incline'))|| ~isempty(findstr(trial, '8 deg uphill')) || ~isempty(findstr(trial, '8 deg'))|| ~isempty(findstr(trial, '8 deg up'))
        ang=8;
%     elseif ~isempty(cell2mat(regexp(trial, '5 deg decline')))|| ~isempty(cell2mat(regexp(trial, '5 deg downhill')))
%         ang=-5;
    else
        ang=input(['What angle (in degrees) was the study run at ', trial, ': ?   ']);
    end
else
    ang=0;
end
